...
-(IBAction)btnSearchPressed:(id)sender{
    ...
    [params setObject:lxtype forKey:@"lx"];
    if ([titleField.text length] > 0) {
        [params setObject:titleField.text forKey:@"q_WJMC"];
    }
    if ([niwenField.text length] > 0) {
        [params setObject:danweiDM forKey:@"q_FBDW"];
    }
    if ([wenHaoField.text length] > 0) {
        [params setObject:wenHaoField.text forKey:@"q_WH"];
    }
    ...
    if([fromDateField.text length] > 0 || [endDateField.text length] >0)
    {
        ...
        {
            [params setObject:fromDateField.text forKey:@"q_RQ1"];
        }
        if ([endDateField.text length] > 0)
        {
            [params setObject:endDateField.text forKey:@"q_RQ2"];
        }
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	NSString *itemTitle = [[resultAry objectAtIndex:indexPath.row] objectForKey:@"WJMC"];
    if (itemTitle== nil) {
        itemTitle = @"";
    }
    ...
    NSString *lwdate = [[resultAry objectAtIndex:indexPath.row] objectForKey:@"JRSJ"];
    if ([lwdate length] > 9) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    TingFawenDetailController *controller = [[TingFawenDetailController alloc] initWithNibName:@"TingFawenDetailController" andFWID:xh];
