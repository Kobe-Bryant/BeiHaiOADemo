...
@interface LaiWenSearchController ()
...
- (void)processWebData:(NSData*)webData
{
    ...
    if (tmpParsedJsonAry && [tmpParsedJsonAry count] > 0) {
        ...
        if (pageInfoDic ) {
            self.totalCount = [[pageInfoDic objectForKey:@"count"] integerValue];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    headerView.text = [NSString stringWithFormat:@"  查询到的来文(%d条)", self.totalCount];
    return headerView;
}
