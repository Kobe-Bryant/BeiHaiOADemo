...
- (NSArray *)queryAllUsersWithSub:(NSString *)deptStr
{
    ...
    NSMutableArray *__block ary = [[NSMutableArray alloc] initWithCapacity:0];
    NSString *sql = [NSString stringWithFormat:@"select * from T_ADMIN_RMS_YH where BMBH='%@' order by PXH ASC;", deptStr];
    if([self hasSubDept:deptStr])
    {
        sql = [NSString stringWithFormat:@"select * from T_ADMIN_RMS_YH where BMBH in (select ZZBH from T_ADMIN_RMS_ZZJG where SJZZXH='%@') order by PXH ASC;", deptStr];
    }
    [self.dbQueue inDatabase:^(FMDatabase *db) {
        FMResultSet *rs = [db executeQuery:sql];
        while ([rs next]) {
